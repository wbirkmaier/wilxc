...
{{-- <!--Get all VM references from IndexController--> --}}
...
{{-- 
        <!--
            ...
            Pull ALL params for a single VM along with variables for resident and powerstate
            $allParams = $xenserver->VM_get_record($vm)
            
            Set residentOn variable for current VM ObjRef
            $residentOn = $allParams["resident_on"]

            Set powerState variable for current VM ObjRef
            $powerState = $allParams["power_state"]
        
            Set hvmBootState variable for current VM ObjRef
            $hvmBootState = $allParams["HVM_boot_policy"]
...
@foreach($vms as $vm)
            ...
            @if($template == '')     
                <tr>
                    ...
                    {{-- <!--Check if a VM is on a host and display row approprialy with link --> --}}
                    @if($residentOn == 'OpaqueRef:NULL')
                        <td>None</td>
                    @else
                        {{-- <!--Create new instance with Credentials Model to get do a host.get_record on Host ObjRef --> --}}
            ...
            {{-- <!--Check power state from variable above and display row approprialy with link--> --}} 
            ...
            @elseif($powerState == 'Running')
                    ...
                    {{-- <!--Check if HVM and Generate shorter Menu--> --}}
                    ...
                    {{-- <!--Check if PV and Generate longer Menu--> --}}
                    @else
                        <td>
                            <div class="btn-group dropdown">
                                <a class="btn btn-primary btn-sm" href="/getVMInfoRef/{{ $vm }}"><i class="fa fa-cog fa-spin fa-fw"></i> Ops</a>
